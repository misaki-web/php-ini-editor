...
class IniEditor {
		...
		if ($this->enable_edit) {
			...
			<script type=\"text/javascript\">
				function addRow(obj, type, isarray) {
					...
					if (isarray == 'array') {
						if (type == 'bool')
							var html = '<label class=\"col-form-label\"><input type=\"text\" class=\"move-input\" size=\"1\"/>'+name+'</label></div><div class=\"col-md-8\"><div class=\"form-group vector\"><span><input class=\"form-control\" type=\"checkbox\" name=\"ini#'+section+'#'+name+'#'+type+'[]\"/> <a href=\"javascript:;\" onclick=\"$(this).parent().parent().insertAfter( $(this).parent().parent().next())\" class=\"down-arr\">&#8595;</a> <a href=\"javascript:;\" onclick=\"$(this).parent().parent().insertBefore( $(this).parent().parent().prev())\" class=\"up-arr\">&#8593;</a>".($this->enable_delete?" <a href=\"javascript:;\" class=\"remove-btn\" onclick=\"$(this).parent().parent().remove();\">X</a>":"")."</span></div>';
						else {
						...
						if (type == 'bool')
							var html = '<label class=\"col-form-label\"><input type=\"text\" class=\"move-input\" size=\"1\"/>'+name+'</label></div><div class=\"col-md-8\"><input class=\"form-control\" type=\"checkbox\" name=\"ini#'+section+'#'+name+'#'+type+'\"/></div>';
						else
	...
	public function getForm() {
		...
		foreach ($conf as $c => $cv) {
			...
			if ($this->enable_add && $this->enable_edit) {
				$html .= '<span><a href="javascript:;" class="btn btn-info" onclick="addRow(this, \'text\');">Add text config</a> <a class="btn btn-info"  href="javascript:;" onclick="addRow(this, \'bool\');">Add Bool config</a> <a class="btn btn-info" href="javascript:;" onclick="addRow(this, \'text\', \'array\');">Add Array config</a></span>'."\n";
			}
			...
			foreach ($cv as $label => $val) {
				...
				if (!is_array($val)) {
					...
					if ($this->enable_edit) {
						$html .= '<a href="javascript:;" onclick="$(this).parent().parent().insertAfter( $(this).parent().parent().next())" class="down-arr">&darr;</a><a href="javascript:;" onclick="$(this).parent().parent().insertBefore( $(this).parent().parent().prev())" class="up-arr">&uarr;</a>';
					...
					if ((isset($c[$label]) && is_bool($c[$label])) || $val == '1' || (!$val && $val != "")) {
						...
						$html .= '<input class="form-control" type="checkbox" name="ini#'.$c.'#'.$label.'#bool" value="1" '.($val?'checked="checked"':'').'/>';
					} else {
					...
					foreach ($val as $k => $v) {
							...
							if (is_bool($val[$k]) || $v == '1' || !$v) {
								...
								$html .= '<input class="form_checkbox" type="checkbox" name="ini#'.$c.'#'.$label.'#bool[]" value="1" '.($v?'checked="checked"':'').'"/>';
							} else {
